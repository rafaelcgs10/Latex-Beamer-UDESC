...
\begin{itemize}
    ...
    \item Coq suporta programação com tipos dependentes e lógica de alta ordem.
\end{itemize}
\end{frame}
...
Para a seguinte definição de \textit{value}:
\begin{verbatim}
...
\begin{frame}[fragile]
\frametitle{\textit{Soundness}}
\begin{itemize}
  \item A principal propriedade de um sistemas de tipos de uma linguagem de programação é \textit{Soundness}.
  \item \textit{Soundness} diz que todos os termos bem tipados nunca entram num estado \textit{stuck}.
  \item Estado \textit{stuck} é um termo que está na forma normal, mas não é um valor (valores aqui são true e false).
  \item \textit{Soundness} pode ser representado por \textit{Progress} + \textit{Preservation}. 
\end{itemize}
\begin{verbatim}
...
\begin{frame}[fragile]
\frametitle{\textit{Preservation}}
\begin{itemize}
  \item \textit{Preservation} diz que o tipo de um termo é preservado durante a redução beta.
  \item A prova do teorema \textit{preservation} também é uma indução estrturada nas regras de derivação, onde há um caso mais complexo referente a regra \textit{AppAbs} que utiliza a substituição, o qual precisa ser provada preservar os tipos. 
  \item Prova-se um lema demostrando a preservação dos tipos na substituição $[x:=s]t$ se o termo $s$ for fechado. 
\end{itemize}
\end{frame}
