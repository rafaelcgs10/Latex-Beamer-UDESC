...
\title[PGFTcRS]{Programação Genética Fortemente Tipada\\ com Restrições Sintáticas}
...
As restrições podem ser custosas computacionalmente, pois podem necessitar comparar partes de
...
\frametitle{Operadores de mutação}
    \begin{itemize}
        \item \textit{New-sub-tree}: troca uma sub-árvore de tipo $\tau$ por uma nova (gerada aleatóriamente)
            que não infrinja o tamanho máximo da árvore e que o tipo únifique com o tipo $\tau$.
        \item \textit{Swap-sub-tree}: Se uma expressão tem duas sub-árvores cujos tipos unifiquem, 
            ex: \mintinline{haskell}{(((Primi "fun") e1) e2)} tal que o tipo de \mintinline{haskell}{e1} unifica com
